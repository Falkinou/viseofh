...
object DecodeStatus {
    const val METADATA_DECODED = "Métadonnées décodées"
    const val KEY_REQUIRED = "Métadonnées décodées - clé DJI requise pour trajectoire"
    const val UNSUPPORTED = "Format non reconnu"
    const val ERROR = "Erreur de décodage"
}
...
class DjiFlightLogDecoder(private val context: Context) {
    ...
    fun decode(log: LogFileEntity, djiApiKey: String = ""): DecodedFlightLog =
        runCatching {
            ...
            val bytes = if (logUri.scheme == "file") {
                logUri.path?.let { FileInputStream(it) }
            } else {
                context.contentResolver.openInputStream(logUri)
            }
                ?.use { it.readBytes() }
    ...
    private fun parseSmartBatteryPercent(bytes: ByteArray): Int? =
        bytes.getOrNull(26)?.toInt()?.and(0xff)?.takeIf { it in 1..100 }

    private fun parseSmartBatteryGroupPercent(bytes: ByteArray): Int? {
        if (bytes.size < 2) return null
        return when (bytes[0].toInt() and 0xff) {
            2 -> bytes.getOrNull(21)?.toInt()?.and(0xff)?.takeIf { it in 1..100 }
            else -> null
        }
    }
