...
class DjiMediaController {
    ...
    private fun ensureSdkReady() {
        ...
        require(state.available) {
            "SDK DJI indisponible : vérifier que l'APK Orange Drone Compagnon complet est installé sur une radiocommande DJI compatible."
        }
        require(state.registered) {
            "SDK DJI non enregistré : connecter la radiocommande à Internet, fermer l’application de vol DJI, puis relancer Orange Drone Compagnon."
        }
        require(state.productConnected) {
            "Drone DJI non connecté : allumer le drone, vérifier la liaison radio, fermer l’application de vol DJI puis relancer Orange Drone Compagnon."
        }
    }
