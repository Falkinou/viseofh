...
class UsbExporter(private val context: Context) {
    ...
    fun testDestination(settings: AppSettings) {
        ...
        val probe = root.createFile("text/plain", ".orange_drone_compagnon_probe")
    ...
    private fun exportRoot(settings: AppSettings): DocumentFile {
        ...
        require(root.exists()) {
            "Dossier USB introuvable : la clé semble démontée, rebranchez-la puis relancez Orange Drone Compagnon."
        }
        return root
    }
    ...
    private fun missionDirectory(settings: AppSettings, log: LogFileEntity): DocumentFile {
        val root = exportRoot(settings)
        val radioDir = root.ensureDirectory("OrangeDroneCompagnon")
    ...
    private fun copySourceToDirectory(
        ...
        openSourceInput(sourceUri).use { input ->
    ...
    private fun openSourceInput(sourceUri: Uri): InputStream? =
        if (sourceUri.scheme == "file") {
            sourceUri.path?.let { FileInputStream(it) }
        } else {
