...
class SyncWorker(
    ...
    override suspend fun doWork(): Result {
        ...
        if (settings.deliveryMode != DeliveryMode.USB) {
            logger.warning("Synchronisation automatique limitée à l'USB. Utilisez MSurvey pour le dépôt sécurisé des logs.")
        ...
        dao.getRetryable().forEach { log ->
            try {
                val detail = when (settings.deliveryMode) {
                    DeliveryMode.FTP,
                    DeliveryMode.SMTP -> "canal historique désactivé"
                    DeliveryMode.USB -> {
